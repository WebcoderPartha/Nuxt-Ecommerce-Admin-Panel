      ...
      <swiper-slide v-if="sliderProduct?.length > 0" v-for="(product, idx) in sliderProduct" :key="idx">
        <div class="border rounded-md p-1 group">
          <div class="relative overflow-hidden">
            <nuxt-img class="hover:scale-125 duration-300" :src="product.image" preload />
            <span
            ...
            <Icon v-if="product?.wishlist?.productId === product.id && authUser?.user?.role === 'customer'"
              @click="wishlistHandler(product.id)" name="heroicons-solid:heart"
          ...
          <div class="text-center flex flex-col gap-4">
            <NuxtLink :to="`/product/${product.slug}`">
              ...
              <p class="text-base font-extrabold" v-if="product.discount !== '0'">
                <span class="line-through text-red-600 text-sm">BDT {{ product.regular_price }}</span> BDT {{
          ...
          <div class="flex md:items-center md:flex-row md:justify-between mt-4 mb-4 px-3 flex-col">
            <button @click="addToCart(product.id)" class="bg-red-500 text-white md:px-2 py-1 rounded-md mb-3 md:mb-0">
              Add to Cart
            </button>
