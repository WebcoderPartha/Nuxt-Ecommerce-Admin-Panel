<template>
  <div class="mb-10 max-w-[1200px] mx-auto">
    ...
    <div v-if="catgoryProduct?.length > 0"
      class="grid grid-cols-2 md:grid-cols-4 items-center justify-center gap-3 lg:grid-cols-5 p-4 md:p-0">
      <!-- Product Item -->
      <div class="shadow-md shadow-gray-300 group rounded-md pb-4" v-for="(hProduct, hinx) in catgoryProduct" :key="hinx">
        ...
        <div class="text-center flex flex-col gap-1 mt-2">
          <h2 class="text-sm font-semibold h-14 text-neutral-500 px-2">
            <NuxtLink class="hover:text-[#79bc62]" :to="`/product/${hProduct.slug}`">{{ hProduct.name.substring(0, 50) +
              '...' }}</NuxtLink>
          </h2>
...
category.value = catpt?.value?.category
// Fetching Category Product
...
const paginationHandler = async (skip) => {
  ...
  category.value = paginatePt?.value?.category
}
//============ Pagination Handler ================//
